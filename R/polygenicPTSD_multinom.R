#' A function to run multinomial regression on categorical hypertension outcome 
#'
#' This function allows you to quickly run and save the required multinomial
#' models for the analysis pipeline.
#'
#' @param data_entry Dataframe input 
#'
#' @return Will return multinom objects for all required models, 
...
#' ancestry principle components. Outcome is 3-level categorical hypertension outcome
...
#'
#'
#' @keywords multinomial regression, categorical outcome
#'
#' @export
#' @examples
...
polygenicPTSD_multinom <- function(data_entry){
  ...
  i = 0 
  for (bp_outcome in c("htn_aha_new", "htn_aha_old")){ #Loop runs through outcomes. Currently just SBP and DBP but will add HTN when ready.
